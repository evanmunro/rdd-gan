...
class GanRDD(object):
    ...
    def evaluate_results(self):
        df_fake = self.generate_data(sample_size = df.shape[0])
        scatterplot = dict(x= ["x"],
                     y= ["y"],
                     samples = 5000, smooth = 0)
        histogram = dict(variables=['x','y','x','y'],
                       nrow=2, ncol=2)
        wgan.compare_dfs(self.df, df_fake, figsize=5, histogram=histogram, scatterplot=scatterplot,
                         save = True, path = "output")
        return 0

    def generate_data(self, save = False, sample_size=1e4):
        s = df.sample(int(sample_size), replace=True)
...
class GanWrapper(object):
    def __init__(self, name, df, outcome, context, lbound, ubound):
        ...
        outcome_scaled, context_scaled = dwrapper.preprocess(df)
        self.outcome = dwrapper.outcome_scaled
        self.context = dwrapper.context_scaled
    ...
    def load_models(self, path = "trained_models"):
        state_dict_critic = torch.load(path + "/" + name + "_C.pth")
        self.critic.load_state_dict(state_dict_critic)
        state_dict_gen = torch.load(path + "/" + name + "_G.pth")
    ...
    def save_model(self, path="trained_models"):
        torch.save(self.critic.state_dict(), path + "/" + name + "_C.pth")
        torch.save(self.generator.state_dict(), path + "/" + name + "_G.pth")
